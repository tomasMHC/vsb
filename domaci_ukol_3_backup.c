...
void kvet(int sirka, int vyska) {
    ...
    left(sirka/2+1);move_down();
    for (int z=0; z<(vyska-(vyska/2)); z++) {
        for (int x=0; x<stonka ; x++) {
            set_green_color();
            draw_pixel();move_right();
        }
        left(stonka);
...
void snake() {
    ...
    for (round = 0; round < 3; round++) {
        if (round<1) {
            for (int i = 0; i < 14; i++) {
                if (x + delka_hada == sirka) {     // prejeti za okraj
                    x = 1;
            ...
            for (int i = 0; i < 20; i++) {
                ...
                if (y == 1 || y == 20) {
                    ...
                    for (int j = 0; j < delka_hada; j++) {  // prejeti za okraj
                        ...
                        move_to(1,1);  
                        ctverec(sirka);
                        move_to(sirka,x_coord);set_green_color(); plna_cara_vertikalna_up(j); set_red_color(); plna_cara_vertikalna_up(1);
                        animate_ms(ms);
                    }
                    y--;
                    ...
                    clear_screen();
                    move_to(1,1);ctverec(sirka);
                    move_to(y,x_coord);set_green_color();plna_cara_vertikalna_up(delka_hada);set_red_color();plna_cara_vertikalna_up(1);
                    animate_ms(ms); 
                    y--;
                } 
            }
