...
int main( int t_narg, char **t_args )
{
    ...
    cv::Mat l_cv_background_img; // empty image

    l_cv_background_img = loadImage( t_args[ 1 ] );

    std::cout << "Opening image: '" << t_args[ 1 ] << "'." << std::endl;
    // load image from file?
    if ( !l_cv_background_img.empty() && l_cv_background_img.channels() == 4)
    {
        std::cout << "Image loaded." << std::endl;
    }
    else if ( l_cv_background_img.channels() != 4 )
    {
    ...
    cv::Mat l_cv_ball_img; // empty image
    l_cv_ball_img = loadImage( "ball.png" );
    std::cout << "Opening image: 'ball.png'." << std::endl;
    // Background OCLImage for kernel
    OCLImage *l_ocl_ball_img = ocl_svm_malloc< OCLImage >();
    fillOCLImageFromMat( *l_ocl_ball_img, l_cv_ball_img );
    cl_int img_sizex = l_ocl_ball_img->m_size.x;
    cl_int img_sizey = l_ocl_ball_img->m_size.y;

    // copy of background
    cv::Mat l_cv_bg_backup_img;
    OCLImage *l_ocl_background_img = ocl_svm_malloc< OCLImage >();
    fillOCLImageFromMat( *l_ocl_background_img, l_cv_background_img );
    l_cv_background_img.copyTo( l_cv_bg_backup_img );

    // create img cv for rotated ball
    cv::Mat l_cv_ball_rotated_img(l_cv_ball_img.rows, l_cv_ball_img.cols, CV_8UC4);
    OCLImage *l_ocl_ball_rotated_img = ocl_svm_malloc<OCLImage>();
    fillOCLImageFromMat(*l_ocl_ball_rotated_img, l_cv_ball_rotated_img);


    //ball rolling from side to side

    cl_int bckg_x = l_ocl_background_img->m_size.x;

    float angle = 0; // rotation angle for the ball, adjust as needed
    cl_int2 ipos = {{ 0, l_cv_background_img.rows - img_sizey - 10 }};
    
    cl_int l_r = l_ocl_ball_img->m_size.x / 2; // radius of the ball
    float angle_step = 0.02f; // rotation angle step for each frame
    cl_int move_x = angle_step * l_r; // horizontal movement based on rotation angle

    while (1) {
        ipos.x += move_x; // move x position based on rotation angle
        angle += angle_step; // increase rotation angle for next frame

        if (ipos.x >= bckg_x - img_sizex) {
            move_x = -move_x;
            angle_step = -angle_step; // reverse rotation direction
            ipos.x = bckg_x - img_sizex; // prevent going out of bounds
        }
        else if (ipos.x <= 0) {
            move_x = -move_x;
            ipos.x = 0;                 // prevent going out of bounds
            angle_step = -angle_step; // reverse rotation direction
        }

        // restore background
        l_cv_bg_backup_img.copyTo( l_cv_background_img );
        gpu_rotate_img( l_program, l_ocl_ball_img, l_ocl_ball_rotated_img, angle );
        gpu_insert_image( l_program, l_ocl_background_img, l_ocl_ball_rotated_img, ipos );
        cv::imshow( "Tennis court", l_cv_background_img );
        cv::waitKey( 1 );
    }
