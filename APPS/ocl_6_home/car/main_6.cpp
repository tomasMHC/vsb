...
int main( int t_narg, char **t_args )
{
    ...
    cv::Mat l_cv_car_img; // empty image
    l_cv_car_img = loadImage( "car.png" );
    std::cout << "Opening image: 'car.png'." << std::endl;
    // Background OCLImage for kernel
    OCLImage *l_ocl_car_img = ocl_svm_malloc< OCLImage >();
    fillOCLImageFromMat( *l_ocl_car_img, l_cv_car_img );
    cl_int img_sizex = l_ocl_car_img->m_size.x;
    cl_int img_sizey = l_ocl_car_img->m_size.y;
    ...
    //car moving from side to side

    gpu_flip_image( l_program, l_ocl_car_img );
    cl_int bckg_x = l_ocl_background_img->m_size.x;


    cl_int2 ipos = {{ -img_sizex, l_cv_background_img.rows - img_sizey - 10 }};
    cl_int move_x = 2;
    ...
    while (1) {
        ...
        gpu_insert_image( l_program, l_ocl_background_img, l_ocl_car_img, ipos );
        cv::imshow( "Route66", l_cv_background_img );
        cv::waitKey( 1 );
    }
