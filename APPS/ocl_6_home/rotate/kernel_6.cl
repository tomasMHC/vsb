...
// **************************************************************************
__kernel void flip_image(__global OCLImage *t_ocl_img)
{
    size_t x = get_global_id(0);
    size_t y = get_global_id(1);

    int width  = t_ocl_img->m_size.x;
    int height = t_ocl_img->m_size.y;
    ...
    uchar4 tmp = t_ocl_img->at4(y, x);
    t_ocl_img->at4(y, x) = t_ocl_img->at4(y, fx);
    t_ocl_img->at4(y, fx) = tmp;
}

__kernel void rotate_img( __global OCLImage *t_ocl_orig_img, __global OCLImage *t_ocl_rotated_img, float t_sin_angle, float t_cos_angle )
{
    size_t x = get_global_id(0);
    size_t y = get_global_id(1);

    if (x >= t_ocl_rotated_img->m_size.x || y >= t_ocl_rotated_img->m_size.y)
        return;

    int cx = t_ocl_orig_img->m_size.x / 2;
    int cy = t_ocl_orig_img->m_size.y / 2;

    int rx = x - t_ocl_rotated_img->m_size.x / 2;
    int ry = y - t_ocl_rotated_img->m_size.y / 2;

    int orig_x = (int)(rx * t_cos_angle + ry * t_sin_angle) + cx;
    int orig_y = (int)(-rx * t_sin_angle + ry * t_cos_angle) + cy;

    if (orig_x >= 0 && orig_x < t_ocl_orig_img->m_size.x && orig_y >= 0 && orig_y < t_ocl_orig_img->m_size.y)
        t_ocl_rotated_img->at4(y, x) = t_ocl_orig_img->at4(orig_y, orig_x);
    else
        t_ocl_rotated_img->at4(y, x) = (uchar4)(0, 0, 0, 0);
}
