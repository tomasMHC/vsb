...
int main() {
    ...
    cout << "Account 2 rate: " << account1->getInterestRate bank->getClientsCount() << endl;
    ...
    // tento účet nemá ir v konštruktore => použije default (0.02)
    Account *accA = bank->createAccount(100, client1);
    accA->Deposit(1000);
    bank->addInterest();
    cout << "accA rate: " << accA->getInterestRate() << " balance: " << accA->getBalance() << endl;

    // tento účet má ir zadané => ignoruje default
    Account *accB = bank->createAccount(101, client1, 0.05);
    accB->Deposit(1000);
    bank->addInterest();
    cout << "accB rate: " << accB->getInterestRate() << " balance: " << accB->getBalance() << endl;
